	...
	for i := 1; i < len(data); i += 2 {
		...
		if data[i] == "*" && data[i - 1] != inconnue && data[i + 1] != inconnue && !IsPowFunc(inconnue, data[i - 1], data[i + 1]) {
			if (strings.Index(data[i - 1], "mat") != -1 || strings.Index(data[i + 1], "mat") != -1) || (IsMat(data[i - 1], vars) || IsMat(data[i + 1], vars)) {
				...
				if ((strings.Index(data[i - 1], "mat") != -1 && strings.Index(data[i + 2], "mat") != -1) || (IsMat(data[i - 1], vars) && IsMat(data[i + 2], vars))) && data[i + 1] == "*" {
					...
					if ((strings.Index(data[i - 1], "mat") != -1 && strings.Index(data[i + 1], "mat") != -1) || (IsMat(data[i - 1], vars) && IsMat(data[i + 1], vars))) {
						data[0] = "'*' is for numbers not matrices"
						return (data)
					}
...
func CalcAddSous(data map[int]string, vars *types.Variable, inconnue string) (map[int]string) {
	...
	for i := 1; i < len(data); i += 2 {
		...
		if data[i] == "+" && data[i - 1] != inconnue && data[i + 1] != inconnue && data[i + 2] != "*" && data[i - 2] != "*" && data[i + 2] != "/" && data[i - 2] != "/" && !IsPowFunc(inconnue, data[i - 1], data[i + 1]) {
			...
			if (strings.Index(data[i - 1], "mat") != -1 && strings.Index(data[i + 1], "mat") != -1) || (IsMat(data[i - 1], vars) && IsMat(data[i + 1], vars)) || (IsMat(data[i - 1], vars) && strings.Index(data[i + 1], "mat") != -1) || (strings.Index(data[i - 1], "mat") != -1 && IsMat(data[i + 1], vars)) {
				if !SizeMat(data[i - 1], data[i + 1], vars) {
					data[0] = "Your matrice must have the same length"
					return (data)
				}
